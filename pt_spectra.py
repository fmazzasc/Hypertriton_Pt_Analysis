...
for i_cent_bins, pt_bins_cent in enumerate(PT_BINS_CENT):
    ...
    for i_split,split in enumerate(SPLIT_LIST):
        ...
        for pt_bins in pt_bins_cent:
            ...
            if SPLIT:
                if i_split==0:
                    h_corrected_ratio.SetBinContent(pt_bin_index, raw_yield/eff[0])
                    h_corrected_ratio.SetBinError(pt_bin_index, raw_yield_error/eff[0])
                else:
                    ...
                    h_corrected_ratio.SetBinContent(pt_bin_index, bin_content/(raw_yield/eff[0]))
        ...
        if FIT==True:
            histo,_ = hp.bw_fit(h_corrected_yields[i_split], bw)
            histo.Write()
